...
#+BEGIN_SRC emacs-lisp
(comment
 ;; No longer need to disable enter key is now bound to right corner key that can be pressed with palm
...
#+BEGIN_SRC emacs-lisp
(comment
 ;; These commands are currently disable as they aren't needed on a 40% keyboard
...
** Auto capitalise sentences (captain)
...
[[https://github.com/prettier/prettier-emacs][prettier-js]] auto format on save. Options can be found [[https://prettier.io/docs/en/options.html][here]].
