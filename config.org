...
(use-package flycheck
  ...
  (set-face-attribute 'flycheck-error nil :underline `(:style line :color ,(doom-color 'red)))
  (set-face-attribute 'flycheck-warning nil :underline `(:style line :color ,(doom-color 'yellow)))
  (set-face-attribute 'flycheck-info nil :underline `(:style line :color ,(doom-color 'green))))
#+END_SRC
...
(defun my/clj-repl ()
  (interactive)
  (my/start-repl "clojure -Sdeps {:deps{compliment{:mvn/version\"0.3.8\"}}}"))
(defun my/lein-repl ()
  (interactive)
  (my/start-repl "lein update-in :dependencies conj [compliment\"0.3.8\"] -- repl"))
...
(defun my/clj-compliment-completion ()
  (interactive)
  (my/clj-eval
   `(do (require '[compliment.core])
        (compliment.core/completions ,(symbol-name (my/clj-symbol-at-point))))))
