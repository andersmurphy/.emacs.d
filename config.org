...
Allows "a" to be used to reused the same dired buffer.
...
(defun contains-substring (string number-of-chars)
  (save-excursion
    (save-match-data
      (goto-char (point-min))
      (search-forward string number-of-chars t))))

(defun clj-insert-ns-form-when-opening-clj-file ()
 (when (and (not (contains-substring "(ns " 10))
            (not (contains-substring "{" 3))
