...
(defun clj-eval (quoted-list)
  (lisp-eval-string
   (replace-regexp-in-string "\\\\." "."
                             (format "%S" quoted-list))))
...
(defun clj-doc-for-symbol ()
  (interactive)
  (clj-eval `(:----Doc------<>
              (clojure.repl/doc ,(symbol-at-point)))))

(defun clj-source-for-symbol ()
  (interactive)
  (clj-eval `(:----Source---<>
              (clojure.repl/source ,(symbol-at-point)))))

(defun clj-javadoc-for-symbol ()
  (interactive)
  (clj-eval `(:----Javadoc--<>
              (clojure.java.javadoc/javadoc ,(symbol-at-point)))))
  ...
  (clj-apply-f-to-ns-and-return-to-point
   (lambda (sym)
     (let* ((sym-name (symbol-name sym))
            (ns (if (string-suffix-p "test" sym-name)
                    (replace-regexp-in-string "-test" "")
                  sym-name))
            (test-ns (if (string-suffix-p "test" sym-name)
                         sym-name
                       (concat sym-name "-test"))))
       (lisp-eval-string ":----Run-Tests--<>")
       (lisp-eval-string (format "(require '%s :reload)" ns))
       (lisp-eval-string (format "(require '%s :reload)" test-ns))
       (lisp-eval-string (format "(clojure.test/run-tests '%s)" test-ns))))))
