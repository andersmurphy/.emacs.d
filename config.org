...
(defun my/try-to-find-project-file (dirname)
  (cond
  ...
  (let ((proc (inferior-lisp-proc)))
    (save-excursion
      ...
      (comint-redirect-send-command-to-process
       command output-buffer proc nil t)
...
(defun my/clj-jump (col file line)
...
(defun my/clj-jump-to-symbol ()
  (interactive)
  (->> (format "(->> (select-keys (meta #\'%s) [:file :line :column])
                    seq
                    sort
                    (map second))"
               (my/clj-symbol-at-point))
       lisp-eval-string
       (my/clj-run-command-read-edn-output "*my/clj-jump*")
       (apply 'my/clj-jump)))
...
(defun my/clj-completions (prefix)
  (-> (my/clj-run-command-read-edn-output
       (my/clj-compliment-completion prefix)
       "*my/clj-completions*")
