...
(defun clj-repl ()
  ...
  (setq inferior-lisp-program
        ;; Need to omit spaces in arg string as inf-lisp
        ;; just does a split on string.
        "clojure -Sdeps {:deps{compliment{:mvn/version\"0.3.8\"}}}"))

(defun lein-repl ()
  (interactive)
  (setq inferior-lisp-program
        "lein update-in :dependencies conj [compliment\"0.3.8\"] -- repl"))
...
(defun switch-to-previous-buffer ()
  (interactive)
  (-> (current-buffer) (other-buffer 1) switch-to-buffer))
...
(defun clj-compliment-completion ()
  ...
  (clj-eval
   `(do (require '[compliment.core])
        (compliment.core/completions ,(symbol-name (symbol-at-point))))))
