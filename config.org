...
(projectile-register-project-type 'npm1 '("package.json" "src" "test") "npm start" "npm test")
(projectile-register-project-type 'npm2 '("package.json" "src" "spec") "npm start" "npm test")
...
(defun custome-projectile-test-suffix (project-type)
  ...
  (cond
   ...
   ((member project-type '(rails-rspec ruby-rspec npm1)) "_spec")
   ((member project-type '(npm2)) "Spec")
...
** Leader key
#+BEGIN_SRC emacs-lisp
(global-set-key (kbd "C-SPC") 'hydra-leader/body)
#+END_SRC
...
(defhydra hydra-leader (:foreign-keys warn :columns 3)
  ...
  ("j" avy-goto-word-1 "jump to" :exit t)
...
[[https://github.com/codesuki/add-node-modules-path][add-node-modules-path]] allows Emacs to find project based installs of
e.g. eslint.
